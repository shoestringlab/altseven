	...
	auth: (function () {
		...
		var _authenticate = function () {
			...
			promise.then(function (secure) {
				a7.ui.views['header'].setState({
					user: a7.model.get('a7').user,
				})
				app.ui.setLayout(secure)
			})
		}
			...
			header.eventHandlers = {
				logout: function () {
					a7.events.publish('auth.logout', {
						callback: app.auth.authenticate,
					})
				},
			}
	...
	ui: (function () {
		...
		return {
			setLayout: function (secure) {
				a7.ui.getNode(
					secure
						? a7.ui.selectors['secureDiv']
						: a7.ui.selectors['anonDiv']
				).style.display = 'block'
				a7.ui.getNode(
					!secure
						? a7.ui.selectors['secureDiv']
